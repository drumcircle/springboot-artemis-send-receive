...
if [[ "$ENABLE_JMX" ]]; then
  ...
  cat << 'EOF' >> $CONFIG_PATH/artemis.profile
    if [ "$1" = "run" ]; then
      JAVA_ARGS="$JAVA_ARGS -Dcom.sun.management.jmxremote=true -Djava.rmi.server.hostname=${JMX_HOST:-localhost} -Dcom.sun.management.jmxremote.port=${JMX_PORT:-1099} -Dcom.sun.management.jmxremote.rmi.port=${JMX_RMI_PORT:-1098} -Dcom.sun.management.jmxremote.ssl=false -Dcom.sun.management.jmxremote.authenticate=false"
    fi
EOF
